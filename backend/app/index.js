...
app.get('/api/bookings/:id', async (req, res) => {
  const bookingId = req.params.id; // Get the ID from the request parameters

  try {
    const [rows] = await pool.query('SELECT * FROM bookings WHERE id = ?', [bookingId]);
    if (rows.length === 0) {
      return res.status(404).send('Booking not found'); // Return 404 if no booking is found
    }
    res.status(200).json(rows[0]); // Return the booking found
  } catch (error) {
